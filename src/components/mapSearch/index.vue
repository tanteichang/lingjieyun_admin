    ...
    <div class="result-panel">
      ...
      <div v-else-if="searchResults.length > 0">
        ...
        <div
          ...
          :key="index"
          class="poi-item"
          :class="{ active: selectedKey === index }"
          @click="locateToPoi(poi.lng, poi.lat, index, false)"
        >
...
const emit = defineEmits<{
  ...
  (e: 'select', value: PoiItem): void;
}>();

// 响应式数据
const searchKeyword = ref(props.keyword); // 搜索关键词
const searchResults = ref<PoiItem[]>([]); // 搜索结果列表
const loading = ref(false); // 加载状态
const hasSearched = ref(false); // 是否执行过搜索
const mapRef = ref(null); // 地图容器DOM引用
const errorMsg = ref(''); // 错误提示
const geoErrorCode = ref<number | null>(null);
const currentLocation = ref(null); // 当前位置信息
const locationLoading = ref(false); // 定位加载状态
const selectedKey = ref(-1); // 选中结果索引
  ...
  localSearch = new window.T.LocalSearch(map, {
    ...
    onSearchComplete: (results) => {
      ...
      searchResults.value = results.pois.map((poi) => {
        const [gcjLat, gcjLng] = ExactGcj02Converter.cgcs2000ToGcj02Exact(
          Number(poi.lonlat.split(',')[1]),
          Number(poi.lonlat.split(',')[0]),
        );
        return {
          name: poi.name,
          address: poi.address,
...
// 定位到指定POI
const locateToPoi = (lng, lat, index, emitSelect = true) => {
  if (!map) return;
  const point = new window.T.LngLat(lng, lat);
  // 地图平移并适度放大到目标点
  ...
  // 添加自定义标注（可选）
  selectedMarker = new window.T.Marker(point);
  map.addOverLay(selectedMarker);
  if (typeof selectedMarker.setTitle === 'function') {
    selectedMarker.setTitle('选中位置');
  }
  if (emitSelect) {
    const selected = searchResults.value[index];
    if (selected) {
...
watch(
  ...
  (value) => {
    ...
    if (value === searchKeyword.value) return;
    searchKeyword.value = value;
    if (props.autoSearch && value.trim() && localSearch) {
      handleSearch();
    }
  },
);
...
onMounted(() => {
  initMap();
  getCurrentLocation();
  if (props.autoSearch && searchKeyword.value.trim()) {
    handleSearch();
  }
});
