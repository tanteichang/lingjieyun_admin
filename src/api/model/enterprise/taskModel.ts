...
export const TASK_STATUS_OPTIONS: Record<TaskStatus, string> = {
...
export interface TaskMemberItem {
  ...
  phone: string;
  is_auth: number;
  realname_status_text: string;
  sign_status: SignStatus;
  sign_status_text: string;
  join_time_formatted: string;
  talent_info_name?: string;
  talent_info_phone?: string;
}

export interface TaskMemberListResult extends Pagination<TaskMemberItem> {
  member_count: number;
  apply_pending_count: number;
  delivery_review_count: number;
}

export interface TaskMemberListResponse extends ApiResponse<TaskMemberListResult> {}
...
export interface BatchRecruitPayload {
  task_id: number; // 任务ID（必填）
  source_url: string; // 导入文件（必填）
  start_row?: number; // 开始行（必填）
}

export type BatchRecruitDetailStatus = 'success' | 'fail';

export interface BatchRecruitDetailItem {
  row_index: number; // Excel 行号
  status: BatchRecruitDetailStatus; // 导入状态
  action: string; // 动作类型（后端返回）
  data: Array<string | number>; // 原始行数据
  message?: string; // 失败原因（失败时可能返回）
  error?: string; // 失败原因（失败时可能返回）
}

export interface BatchRecruitResult {
  log_id: number; // 导入日志ID
  batch_no: string; // 批次号
  total_count: number; // 总条数
  success_count: number; // 成功条数
  fail_count: number; // 失败条数
  import_details: BatchRecruitDetailItem[]; // 明细
  duration: number; // 耗时（秒）
}

export type BatchRecruitResponse = ApiResponse<BatchRecruitResult>;
...
export interface RecruitPayload {
  ...
  id_card_front: string; // 身份证正面（必填）
  id_card_back: string; // 身份证反面（必填）
  sign_method: SignMethod; // 签约方式（必填）
}

export type RecruitResponse = ApiResponse<{
  talent_pool_id: number; // 人才池ID
  member_added: boolean; // 是否成功添加人才池成员
}>;
